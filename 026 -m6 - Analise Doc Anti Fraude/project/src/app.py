...
def configura_interface():
    st.title("Upload de Arquvos DIO - Desafio 1 - Azure - Fake Docs")
    uploaded_file = st.file_uploader("Escolha um arquivo", type=["png", "jpg", "jpeg"])

    if uploaded_file is not None:
        fileName = uploaded_file.name
        #enviar para o blob storage Azure
...
def show_image_and_validation(blob_url, credit_card_info):
    ...
    if credit_card_info and credit_card_info["card_name"]:
        st.markdown(f"<h1 style='color :green;'>Cartão Valido</h1>", unsafe_allow_htm=True)
        st.write(f"Nome do Titular: {credit_card_info['card_name']}")
        st.write(f"Banco Emissor: {credit_card_info['bank_name']}")
        st.write(f"Data de Validade: {credit_card_info['expiry_date']}")
    else:
        st.markdown(f"<h1 style='color :red;'>Cartão Invalido</h1>", unsafe_allow_htm=True)
